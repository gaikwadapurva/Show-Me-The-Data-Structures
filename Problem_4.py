...
sub_child.add_user("thequeen")

sub_child.add_group(sub_sub_child)
assert(is_user_in_group("sub2child", sub_child) == True)
child.add_group(sub_child)
parent.add_group(child)
assert(is_user_in_group("thequeen", parent) == True)

assert(is_user_in_group('thequeen', 'group') == "Invalid group!")
assert(is_user_in_group('thequeen') == 'Group input missing')
